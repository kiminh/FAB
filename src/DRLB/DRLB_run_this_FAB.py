...
def statistics(B_t, origin_t_spent, origin_t_market_prices, origin_t_win_imps,
        ...
        if B_t[t] - origin_t_spent <= 0 or remain_auc_num[t] - origin_t_auctions <= 0:
            ...
            for i in range(len(auc_t_datas)):
                ...
                if remain_auc_num[t] - temp_t_auctions >= 0:
                    temp_t_market_prices += auc_t_datas[i, 2]
            ...
            t_market_prices = temp_t_market_prices if temp_t_market_prices > 0 else 0
            t_win_imps = temp_t_win_imps
            t_clks = temp_t_clks
            reward_t = temp_reward_t
            profit_t = temp_profit_t
        else:
            t_spent, t_market_prices, t_win_imps, t_auctions, t_clks, reward_t, profit_t \
                = origin_t_spent, origin_t_market_prices, origin_t_win_imps, origin_t_auctions, origin_t_clks, origin_reward_t, origin_profit_t
    else:
        t_auctions = 0
        t_spent = 0
        t_market_prices = 0
    ...
    return t_win_imps, t_spent, t_market_prices, t_auctions, reward_t, t_clks, profit_t
...
def state_(budget, auc_num, auc_t_datas, auc_t_data_pctrs, lamda, action, B_t, time_t, remain_auc_num):
    ...
    t_market_prices = np.sum(auc_t_datas[:, 2]) # 当前时段市场价格
    ...
    if time_t == 0:
        if remain_auc_num[0] > 0:
            t_win_imps, t_spent, t_market_prices, t_auctions, reward_t, t_clks, profit_t \
                = statistics(B_t, t_spent, t_market_prices, t_win_imps, t_auctions, t_clks, reward_t, profit_t, auc_t_datas, bid_arrays, remain_auc_num, 0)
        else:
            t_win_imps = 0
            t_spent = 0
            t_market_prices = 0
        ...
        if remain_auc_num[time_t - 1] > 0:
            t_win_imps, t_spent, t_market_prices, t_auctions, reward_t, t_clks, profit_t \
                = statistics(B_t, t_spent, t_market_prices, t_win_imps, t_auctions, t_clks, reward_t, profit_t, auc_t_datas, bid_arrays, remain_auc_num, time_t - 1)
        else:
            t_auctions = 0
            t_spent = 0
            t_market_prices = 0
    ...
    return state_t, lamda, B_t, net_reward_t[0][0], origin_reward_t, profit_t, t_clks, bid_arrays, remain_auc_num, t_win_imps, t_real_imps, t_real_clks, t_spent, t_market_prices, done
...
def run_env(budget_para):
    ...
    for episode in range(config['train_episodes']):
        ...
        for t in range(96):
            ...
            if t == 0:
                init_action = 0
                state_t, lamda_t, B_t, reward_t, origin_reward_t, profit_t, t_clks, bid_arrays, t_remain_auc_num, t_win_imps, t_real_imps, t_real_clks, t_spent, t_market_prices, done = state_(budget, auc_num, auc_t_datas, auc_t_data_pctrs,
                ...
                t_win_imps_next, t_real_imps_next, t_real_clks_next, t_spent_next, t_market_prices_next, done_next \
                ...
                state_t, lamda_t, B_t, reward_t, origin_reward_t, profit_t, t_clks, bid_arrays, t_remain_auc_num, t_win_imps, t_real_imps, t_real_clks, t_spent, t_market_prices, done = state_(budget, auc_num, auc_t_datas, auc_t_data_pctrs,temp_lamda_t_next, action_t_next, temp_B_t_next, time_t, temp_remain_t_auctions)
            ...
            episode_market_prices += t_market_prices
            ...
            reward_t = episode_clks / episode_real_clks - (episode_real_clks - episode_clks) / episode_real_clks - episode_spent / episode_market_prices

            # print(action)
            transition = np.hstack((state_t, action, reward_t/1e3, state_t_next, done))
            RL.store_transition(transition)  # 存储在DRLB的经验池中
    ...
    columns = ['real_imps', 'win_imps', 'clks', 'real_clks', 'rewards', 'budget', 'spent', 'CPM']

    test_records_array_df = pd.DataFrame(data=test_records_array)
    test_records_array_df.to_csv('result2/' + data_type['campaign_id'] + data_type['type'] + '/test_episode_results_' + str(budget_para) + '.csv')

    test_actions_array_df = pd.DataFrame(data=test_actions_array)
    test_actions_array_df.to_csv(
        'result2/' + data_type['campaign_id'] + data_type['type'] + '/test_episode_actions_' + str(budget_para) + '.csv')

    # all sample
    # test_all_records_array_df = pd.DataFrame(data=test_all_records_array)
    # test_all_records_array_df.to_csv(
    #     'result2/' + data_type['campaign_id'] + data_type['type'] + '/all_sample/test_episode_results_' + str(budget_para) + '.csv')
    #
    # test_all_actions_array_df = pd.DataFrame(data=test_all_actions_array)
    # test_all_actions_array_df.to_csv(
    #     'result2/' + data_type['campaign_id'] + data_type['type'] + '/all_sample/test_episode_actions_' + str(budget_para) + '.csv')
    #
    # # down sample
    # test_down_records_array_df = pd.DataFrame(data=test_down_records_array)
    # test_down_records_array_df.to_csv(
    #     'result2/' + data_type['campaign_id'] + data_type['type'] + '/down_sample/test_episode_results_' + str(budget_para) + '.csv')
    #
    # test_down_actions_array_df = pd.DataFrame(data=test_down_actions_array)
    # test_down_actions_array_df.to_csv(
    #     'result2/' + data_type['campaign_id'] + data_type['type'] + '/down_sample/test_episode_actions_' + str(budget_para) + '.csv')

    action_df = pd.DataFrame(data=episode_action_records)
    action_df.to_csv('result2/' + data_type['campaign_id'] + data_type['type'] + '/train_episode_actions_' + str(budget_para) + '.csv')

    result_data_df = pd.DataFrame(data=result_data, columns=columns)
    result_data_df.to_csv('result2/' + data_type['campaign_id'] + data_type['type'] + '/train_episode_results_' + str(budget_para) + '.csv')
...
def run_down_test(budget_para, original_ctr):
    ...
    for t in range(96):
        ...
        if t == 0:
            init_action = 0
            state_t, lamda_t, B_t, reward_t, origin_reward_t, profit_t, t_clks, bid_arrays, t_remain_auc_num, t_win_imps, t_real_imps, t_real_clks, t_spent, t_market_prices, done = state_(budget, auc_num, auc_t_datas, auc_t_data_pctrs, init_lamda, init_action, B_t, time_t, remain_auc_num)  # 1时段
            action = RL.choose_best_action(state_t)
        ...
        reward_t = (2 * episode_clks - episode_real_clks) / episode_real_clks - episode_spent / episode_market_prices
    ...
    print('测试集中：真实曝光数{}, 赢标数{}, 共获得{}个点击, 真实点击数{}, '
          '奖励{}, 预算{}, 花费{}, CPM{}, {}'.format(episode_imps, episode_win_imps, episode_clks, episode_real_clks,
                                               episode_rewards, budget, episode_spent, episode_spent / episode_win_imps, datetime.datetime.now()))

    temp_result = [episode_imps, episode_win_imps, episode_clks, episode_real_clks,
                   episode_rewards, budget, episode_spent, episode_spent / episode_win_imps]
...
def run_all_test(budget_para, original_ctr):
    ...
    for t in range(96):
        ...
        if t == 0:
            init_action = 0
            state_t, lamda_t, B_t, reward_t, origin_reward_t, profit_t, t_clks, bid_arrays, t_remain_auc_num, t_win_imps, t_real_imps, t_real_clks, t_spent, t_market_prices, done = state_(
                budget, auc_num, auc_t_datas, auc_t_data_pctrs, init_lamda, init_action, B_t, time_t,
                remain_auc_num)  # 1时段
            action = RL.choose_best_action(state_t)
