...
def run_env(budget_para):
    ...
    test_data = pd.read_csv(data_type['data_path'] + data_type['campaign_id'] + str(data_type['fraction_type'])
                            + '/test_data.csv', header=None).drop([0])
    ...
    test_all_sample = pd.read_csv(data_type['data_path'] + data_type['campaign_id'] + str(data_type['fraction_type'])
                            + '/test_all_sample.csv', header=None).drop([0])
    ...
    test_down_sample = pd.read_csv(data_type['data_path'] + data_type['campaign_id'] + str(data_type['fraction_type'])
                                  + '/test_down_sample.csv', header=None).drop([0])
    ...
    for episode in range(config['train_episodes']):
        ...
        if (episode > 0) and ((episode + 1) % config['observation_episode'] == 0):
            ...
            test_all_result, test_all_actions, test_hour_clks = test_env(config['test_budget'] * budget_para, budget_para,
                                                                 test_all_sample, eCPC)
            test_all_records.append(test_all_result)
            e_all_actions.append(test_all_actions)

            test_down_result, test_down_actions, test_hour_clks = test_env(config['test_budget'] * budget_para, budget_para,
    ...
    e_actions_df = pd.DataFrame(data=e_actions)
    e_actions_df.to_csv(
        log_path + '/result_reward_1/' + str(fraction_type) + '/test_episode_actions_' + str(budget_para) + '.csv')

    # test_all_sample
    test_all_records_df = pd.DataFrame(data=test_all_records,
                                   columns=['profits', 'budget', 'cost', 'clks', 'real_clks', 'bids', 'imps', 'cpm',
                                            'break_time_slot'])
    test_all_records_df.to_csv(
        log_path + '/result_reward_1/' + str(fraction_type) + '/all_sample/test_episode_results_' + str(budget_para) + '.csv')

    e_all_actions_df = pd.DataFrame(data=e_all_actions)
    e_all_actions_df.to_csv(
        log_path + '/result_reward_1/' + str(fraction_type) + '/all_sample/test_episode_actions_' + str(budget_para) + '.csv')

    # test_down_sample
    test_down_records_df = pd.DataFrame(data=test_down_records,
                                       columns=['profits', 'budget', 'cost', 'clks', 'real_clks', 'bids', 'imps', 'cpm',
                                                'break_time_slot'])
    test_down_records_df.to_csv(
        log_path + '/result_reward_1/' + str(fraction_type) + '/down_sample/test_episode_results_' + str(
            budget_para) + '.csv')

    e_down_actions_df = pd.DataFrame(data=e_down_actions)
    e_down_actions_df.to_csv(
        log_path + '/result_reward_1/' + str(fraction_type) + '/down_sample/test_episode_actions_' + str(
            budget_para) + '.csv')
