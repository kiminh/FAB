...
def list_metrics(budget_para, algo, result_directory):
    bid_records = pd.read_csv(result_directory + algo + '_bids_' + str(budget_para) + '.csv', header=None).drop([0])
...
def action_distribution(budget_para, algo, result_directory):
    bid_records = pd.read_csv(result_directory + algo + '_bids_' + str(budget_para) + '.csv', header=None).drop([0])
    bid_records.iloc[:, 0] = bid_records.iloc[:, 0].astype(float)
    bid_records.iloc[:, 1:4] = bid_records.iloc[:, 1:4].astype(int)
...
def clk_frequency(budget_para, algo, result_directory):
    bid_records = pd.read_csv(result_directory + algo + '_bids_' + str(budget_para) + '.csv', header=None).drop([0])
    bid_records.iloc[:, 0] = bid_records.iloc[:, 0].astype(float)
...
def ctr_statistics(budget_para, algo, result_directory):
    bid_records = pd.read_csv(result_directory + algo + '_bids_' + str(budget_para) + '.csv', header=None).drop([0])
    bid_records.iloc[:, 0] = bid_records.iloc[:, 0].astype(float)
    bid_records.iloc[:, 0] = np.ceil(bid_records.iloc[:, 0].values)  # 向下取整
    bid_records.iloc[:, 0] = bid_records.iloc[:, 0].astype(int)
...
#     clk_frequency(budget_para, algo, result_directory)
#
# print('\n##########Time slots’ctr statistics##########')
# for budget_para in budget_paras:
#     print('\n------budget_para:{}------'.format(budget_para))
#     ctr_statistics(budget_para, algo, result_directory)

print('\n##########List Metrics##########')
for budget_para in budget_paras:
    print('\n------budget_para:{}------'.format(budget_para))
    list_metrics(budget_para, algo, result_directory)

# print('\n##########Action Distribution##########')
# for budget_para in budget_paras:
#     print('\n------budget_para:{}------'.format(budget_para))
#     action_distribution(budget_para, algo, result_directory)
