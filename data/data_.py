...
def generate_data(campaign_id, fraction_type):
    ...
    print(len(train_data_df), np.sum(train_data_df.iloc[:, 0]), np.sum(train_data_df.iloc[:, 1]))
    print(len(test_data_df), np.sum(test_data_df.iloc[:, 0]), np.sum(test_data_df.iloc[:, 1]))

    test_clks = np.sum(test_data_df.iloc[:, 0])
